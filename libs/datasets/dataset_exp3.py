...
class ig_dataset(torch.utils.data.Dataset):
    def __init__(self, args):
        self.args = args
        self.data_ids = [i for i in range(1, self.args.gail_experts_data_length)]
        ...
        self.dataset_name = 'dataset_cabinet_mixed_clean'

        for item in self.data_ids:
            self.data_buff_states[item] = np.load('dataset/{0}/{1}_states.npy'.format(self.dataset_name, item))
            self.data_buff_actions[item] = np.load('dataset/{0}/{1}_actions.npy'.format(self.dataset_name, item))
    ...
    def __getitem__(self, index):
        ...
        if frame_idx >= (self.seq_length - 1):
            ...
            handle_pos_1 = inputs_raw[:, 30:33]
            handle_pos_2 = inputs_raw[:, 33:36]

            inputs = np.concatenate((pos_1 - base_1, pos_2 - base_2, obj_pos - pos_1, obj_pos - pos_2, obj_ori, handle_pos_1 - pos_1, handle_pos_1 - pos_2, handle_pos_2 - pos_1, handle_pos_2 - pos_2), axis=1)
        ...
        action_gt = torch.FloatTensor(np.array(self.data_buff_actions[video_idx][frame_idx + 1])).view(-1)
        action_gt[:3] *= 1000.0
